...
Данный код выполняет такие задачи как выводит длину предложения переводит предложение в нижний регистр подсчитывает количество гласных в предложении замените все слова "ugly" на "beauty" проверяет, начинается ли предложение с "The" и заканчивается на "end". В коде пятнадцать функция `print` которая выводит предложения и переменные. Метод `lower()` переводить строки в нижний регистр.

1. `st = input("enter sentences: ") `: Переменная `st` имеет строковое значение функция `input` позволяет пользователю вводить предложение в консоль.
2. `if "The" in st:` Конструкция `if` проверяет начинается ли предложение с "The".
3. ` print("contains 'The'")`: Функция `print()` выводит сообщение в консоль.
4. ` if "end" in st:`: Конструкция `if` проверяет заканчивается ли предложение на "end".
5. ` print("contains 'end'")`: Функция `print()` выводит сообщение в консоль.
6. `a = st.replace("ugly", "beauty")`: Метод `replace()` заменяет все слова "ugly" на "beauty".
7. `for char in st:`: Цикл `for`выведите длину предложения.
8. ` if char in "aeiouAEIOU":`: Конструкция `fi` считает сколько гласных в предложении.
9. `a = st.replace("ugly", "beauty")`: Метод `replace()` заменяет все слова "ugly" на "beauty".
10. `print("Vowel count:", count)`: Функция `print()` выводит сообщение в консоль.
11. `print("line length:",len(st))`: Функция `print()` выводит сообщение в консоль.
12. `print(st.lower())`: Функция `print()`выводит в консоль метод `lower()` переводить строки в нижний регистр.
13. `print(a)`: Функция `print()`выводит в консоль переменную.

## Общие выводы по теме 3

В Данной тема я изучила операторов, условия, циклы. 
В языке Python есть множество операторов, условий и циклов, которые используются для решения разных задач. Данная тема мне показалась более сложной в изучении чем предыдущая.
